...
class ChatController extends GetxController {
  ...
  void sendMessage(String text, {String type = 'text'}) async {
    if (text.isNotEmpty) {
      ...
      usersRef.doc(currentUser!.uid).collection('inbox').doc(receiver_id).set(message.toMap());
      usersRef.doc(receiver_id).collection('inbox').doc(currentUser!.uid).set(message.toMap());
